...
| `4.2` | Outbound text | Implemented | `sendMessageChat4000()` is wired through the normal outbound adapter, and queued chat4000 replies are now recovered from OpenClaw's `delivery-queue` while connected. |
...
| `5.4` | Outbound adapter | Partial | `sendText()` and URL-based `sendMedia()` exist; queued/deferred chat4000 replies are replayed from `delivery-queue`, but richer content and full host-managed streaming hooks still do not exist. |
