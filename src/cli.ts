...
export function registerChat4000Cli(api: PluginApiLike): void {
  api.registerCli?.(
    ({ program }) => {
      ...
      chat4000
        .command("pair")
        .description("Configure chat4000 (if needed) and start a pairing session for a client")
...
async function runPairCommand(api: PluginApiLike, opts: PairCommandOptions): Promise<boolean> {
  const cfg = loadConfig(api);
  const before = resolveChat4000Account({
    cfg: cfg as { channels?: Record<string, unknown> },
    accountId: opts.account,
  });
  ...
  if (shouldSkipPairing(opts)) {
    ...
    output.write('Next step: "openclaw chat4000 pair"\n');
    return true;
  }
  ...
  const after = resolveChat4000Account({
...
function restartHint(beforeConfigured: boolean): string {
  if (beforeConfigured) {
    return "Gateway already serves chat4000 — no restart needed.\n";
  }
  return 'Restart the gateway to start serving chat4000: "openclaw gateway restart"\n';
}
