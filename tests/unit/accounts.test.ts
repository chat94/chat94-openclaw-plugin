...
import { resolveChat4000Account, hasConfiguredState } from "../../src/accounts.js";
...
describe("accounts", () => {
  ...
  function makeOpenClawHome(): string {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), "chat4000-accounts-"));
    tempDirs.push(dir);
  ...
  it("derives group id from config key", () => {
    ...
    expect(account.relayUrl).toBe("wss://relay.chat4000.com/ws");
  ...
  it("group key env vars override config", () => {
    ...
    process.env.CHAT4000_GROUP_KEY = envKey.toString("base64url");
  ...
  it("hasConfiguredState checks env vars", () => {
    ...
    expect(hasConfiguredState({
      CHAT4000_GROUP_KEY: groupKeyB64,
    })).toBe(true);
  });

  it("empty config returns default relay URL", () => {
    const account = resolveChat4000Account({ cfg: {} });
    expect(account.relayUrl).toBe("wss://relay.chat4000.com/ws");
    expect(account.configured).toBe(false);
  });
  ...
  it("loads the long-lived key from the plugin state file", () => {
    ...
    expect(account.groupKeyBytes.equals(groupKey)).toBe(true);
    expect(account.keyFilePath).toContain("plugins/chat4000/keys/default.json");
  });
});
